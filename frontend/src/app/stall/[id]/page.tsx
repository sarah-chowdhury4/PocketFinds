...
import { useStallById } from "@/hooks/useStalls"
...
export default function StallPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { stall, isLoading } = useStallById(resolvedParams.id)
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        setMenuLoading(true)
        const res: any = await menuAPI.getStallMenu(stall._id)
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pb-20 md:pb-8">
        ...
        <div className="relative h-48 md:h-72 w-full">
          <Image src={(stall?.image) || "/placeholder.svg"} alt={stall?.stall_name || "Stall"} fill className="object-cover" />
          ...
          {/* Action Buttons (share only; favorites/bookmarks removed until API-backed) */}
          ...
          <div className="absolute bottom-4 left-4 flex gap-2">
            {stall?.discount && stall.discount > 0 && (
              <Badge className="bg-primary text-primary-foreground">{stall.discount}% OFF</Badge>
            )}
            <Badge
              variant={stall?.isOpen ? "default" : "destructive"}
              className={stall?.isOpen ? "bg-success text-success-foreground" : ""}
            >
        ...
        <div className="container px-4 py-6">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold">{stall?.stall_name || ""}</h1>
              ...
              <div className="flex items-center gap-4 mt-3">
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-bold text-lg">{stall?.rating || 0}</span>
                  <span className="text-muted-foreground">({stall?.reviewCount || 0} reviews)</span>
                </div>
              </div>

              <div className="flex flex-wrap gap-2 mt-3">
                {/* categories not in schema; omit for now */}
              </div>
            </div>
          ...
          {/* Offers */}
          {(stall?.offer || (stall?.discount && stall.discount > 0)) && (
            ...
            <TabsContent value="menu" className="mt-6">
              <div className="space-y-4">
                {menuLoading ? (
                  ...
                  <div className="text-muted-foreground">No menu items.</div>
                )}
              </div>
            </TabsContent>
